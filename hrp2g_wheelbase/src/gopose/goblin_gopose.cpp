...
private:
  ...
  float odom_interval, v_linear_max, v_angular_max;
    int odom_counter = 0;
  float Kp,Kd,Kpa,Kda,Kpa_param,Kda_param,Kp_param, Kd_param , thre_param, thre;
  float a_vel;
public:
    void init()
    {
        ...
        nh_.setParam("/goblin/Kpa",Kpa_param);
        nh_.setParam("/goblin/Kda",Kda_param);
        nh_.setParam("/goblin/thre",thre_param);
...
bool Gopose::Gopose_srv(hrp2g_wheelbase::Gopose::Request &req,
                hrp2g_wheelbase::Gopose::Response &res)
{
  if((fabs(req.x/req.time) > v_linear_max) || (fabs(req.theta/req.time) > v_angular_max))
    {
        res.status = 0;
    }
        else
    {
        ...
        Kpa = Kpa_param/req.time;
        Kda = Kda_param/(req.time*odom_interval);
        a_vel = req.time;
    }
}
...
void Gopose::OdomCallBack(const geometry_msgs::TwistConstPtr& twist)
{
    if(tmp_req.time > 0)
    {
        ...
        nh_.getParam("/goblin/thre",thre_param);
        thre = thre_param;
        ...
        if(odom_counter > 5)
        {
            ...
            float v_a = Kpa*err_a - Kda*d_err_a;
            v_l = v_l==0? v_l+0.01:v_l;
            cmd_vel.linear.x = v_l;
            if(fabs(tmp_req.theta) > thre || fabs(tmp_req.x)>0.1)
              cmd_vel.angular.z = v_a;
            else
              cmd_vel.angular.z = err_a;
            if(fabs(cmd_vel.angular.z) > fabs(tmp_req.theta/a_vel))
              {
                cmd_vel.angular.z = cmd_vel.angular.z/fabs(cmd_vel.angular.z) * 
                  (fabs(tmp_req.theta))/a_vel;
              }
