...
public:
    void init()
    {
        ...
        endgopose = n_.serviceClient<hrp2g_wheelbase::Gopose>("/goblin/gopose");
...
void TeleopUAVJoy::JoyCallback(const sensor_msgs::Joy::ConstPtr& joy)
{
    ...
    else if(joy->buttons[PS3_BUTTON_REAR_RIGHT_1]&&joy->buttons[PS3_BUTTON_REAR_LEFT_1])
    {
    ...
    //taking over control from the gopose
    if(joy->buttons[PS3_AXIS_BUTTON_ACTION_SQUARE])
      {
        hrp2g_wheelbase::Gopose srv;
        srv.request.x  = 0;
        srv.request.theta = 0;
        srv.request.time = 0.5;
        endgopose.call(srv);
      }
